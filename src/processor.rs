...
impl Processor {
    ...
    fn process_init_escrow(
        ...
        /// We need an account to store the escorw info
    ...
    fn process_exchange(
        ...
        if amount_expected_by_taker != pda_temp_token_account_info.amount {
            return Err(EscrowError::ExpectedAmountMismatched.into());
        }

        let initializers_main_account = next_account_info(account_info_iter)?;
        let initializers_receiving_token_account = next_account_info(account_info_iter)?;
        let escrow_account = next_account_info(account_info_iter)?;
        ...
        if escrow_info.initializer_token_to_receive_account_pubkey
            != *initializers_receiving_token_account.key
        {
            return Err(ProgramError::InvalidAccountData);
        }
        ...
        invoke(
            ...
            &[
                ...
                initializers_receiving_token_account.clone(),
                taker.clone(),
                token_program.clone(),
            ],
        )?;
